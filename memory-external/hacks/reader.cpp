...
float CC4::get_blow_time() {
	return g_game.process->read<float>(get_planted() + updater::offsets::m_flC4Blow);
}
...
void CGame::loop() {
	...
	viewAngles = process->read<Vector3>(base_client.base + updater::offsets::dwViewAngles);
